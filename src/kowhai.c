...
int _get_setting_offset(struct kowhai_node_t* tree, int num_symbols, union kowhai_symbol_t* symbols, int symbols_matched, int* finished, int* steps, int* finished_on_leaf)
{
    int offset = 0;
    do
    {
        ...
        switch (tree->type)
        {
            case NODE_TYPE_BRANCH:
            {
                // recurse into branch
                int _steps = 0;
                int temp = _get_setting_offset(tree + 1, num_symbols, symbols, symbols_matched, finished, &_steps, finished_on_leaf);
                if (temp == -1)
                {
#ifdef KOWHAI_DBG
                ...
                else
                {
                    ...
                    {
                        int __steps = 0;
                        if (array_index > tree->count - 1)
                        {
#ifdef KOWHAI_DBG
                            printf(KOWHAI_ERR" symbol match error, symbol array_index: %d, node count: %d\n", array_index, tree->count);
#endif
                            return -1;
                        }
                        temp += _get_branch_size(tree + 1, &__steps) * array_index;
                    }
                    // leaf array
                    if (*finished_on_leaf)
                    {
                        *finished_on_leaf = 0;
                        ...
                        if (array_index > 0)
                        {
                            struct kowhai_node_t* leaf = tree + _steps;
...
{
    int finished = 0, steps = 0, finished_on_leaf = 0;
    return _get_setting_offset(tree, num_symbols, symbols, 0, &finished, &steps, &finished_on_leaf);
}
