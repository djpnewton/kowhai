...
namespace kowhai_sharp
{
    public class KowhaiSerialize
    {
        ...
        public static extern int kowhai_serialize(IntPtr descriptor, IntPtr data, int data_size, IntPtr target_buffer, ref int target_size, kowhai_get_symbol_name_t get_name);
        ...
        public static int Serialize(Kowhai.kowhai_node_t[] descriptor, byte[] data, out string target, int targetBufferSize, kowhai_get_symbol_name_t getName)
        {
            ...
            GCHandle hTarget = GCHandle.Alloc(data, GCHandleType.Pinned);
            int result = kowhai_serialize(hDesc.AddrOfPinnedObject(), hData.AddrOfPinnedObject(), data.Length, hTarget.AddrOfPinnedObject(), ref targetBufferSize, getName);
            hTarget.Free();
            hData.Free();
            hDesc.Free();
            ASCIIEncoding enc = new ASCIIEncoding();
            target = enc.GetString(targetBuf);
            return result;
        }
    }
}
